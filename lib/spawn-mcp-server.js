...
//   onRequestAttention({ sessionId?, reason?, clear? }) -> Promise<{ sessionId, cliSessionId?, needsAttention }>
...
function getToolDefs(onSpawn, onMarkDone, onRename, onSendMessage, onRequestAttention) {
  var tools = [];
  ...
  if (typeof onRequestAttention === "function") {
    tools.push({
      name: "request_user_attention",
      description:
        "Flag the calling session as needing user input. Clay's sidebar " +
        "highlights the session with an attention indicator (colour + " +
        "badge) so the user can spot which spawned session is blocked. " +
        "Auto-clears the moment the user opens (switches to) the session.\n" +
        "\n" +
        "USE THIS TOOL when work is paused because:\n" +
        "  - You hit an approval gate (e.g. /jira plan-mode plan ready for review)\n" +
        "  - You need a decision from the human (which option, which API key, etc.)\n" +
        "  - You ran into an ambiguous problem and want to ask before continuing\n" +
        "  - You're done with everything you can do without further input\n" +
        "\n" +
        "Do NOT use this tool for routine progress updates — that's what " +
        "the jsonl watcher's response previews are for. Reserve it for " +
        "moments when the user genuinely needs to look.\n" +
        "\n" +
        "session_id: optional Clay localId. Omit to flag the calling " +
        "session (the common case).\n" +
        "\n" +
        "reason: optional short string (max 80 chars) that becomes the " +
        "sidebar tooltip on the attention indicator. Examples: \"plan " +
        "ready for approval\", \"blocked on JIRA cloud ID\", \"deploy to " +
        "us-test confirmed?\".\n" +
        "\n" +
        "clear: pass true to clear the flag without waiting for the user " +
        "to open the session (e.g. you decided to proceed without input).",
      inputSchema: buildShape({
        session_id: {
          type: "number",
          description:
            "Clay localId of the session to flag. Omit to flag the " +
            "currently calling session.",
        },
        reason: {
          type: "string",
          description: "Short reason (≤80 chars) shown on the sidebar indicator.",
        },
        clear: {
          type: "boolean",
          description: "Pass true to clear the attention flag.",
        },
      }, []),
      handler: function (args) {
        var sessionId = (args && typeof args.session_id === "number") ? args.session_id : null;
        var reason = (args && typeof args.reason === "string") ? args.reason.trim().substring(0, 80) : "";
        var clear = !!(args && args.clear);
        var callingCliSessionId = (args && args.__callingCliSessionId) || null;
        return Promise.resolve()
          .then(function () {
            return onRequestAttention({
              sessionId: sessionId,
              reason: reason,
              clear: clear,
              __callingCliSessionId: callingCliSessionId,
          ...
          .then(function (result) {
            var localId = result && result.sessionId;
            var flag = result && result.needsAttention;
            var summary;
            if (clear) {
              summary = "Cleared attention flag on session #" + localId + ".";
            } else if (flag) {
              summary = "Flagged session #" + localId + " as needing attention" +
                (flag.reason ? " (" + flag.reason + ")" : "") + ".";
            } else {
              summary = "Session #" + localId + " attention state unchanged.";
            }
            var trackingLine = "[clay-sessions/request_user_attention] localId=" + localId +
              " cliSessionId=" + ((result && result.cliSessionId) || "(none)") +
              " needsAttention=" + (flag ? JSON.stringify(flag) : "null");
            return {
              content: [
                { type: "text", text: summary },
                { type: "text", text: trackingLine },
          ...
          .catch(function (err) {
            return {
              content: [{ type: "text", text: "Error setting attention flag: " + (err && err.message || err) }],
              isError: true,
            };
          });
      },
