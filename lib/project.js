    ...
    if (!isMate) {
      try {
        ...
        var spawnToolDefs = spawnMcp.getToolDefs(function onSpawn(spawnArgs) {
          ...
          if (mode === "tui") {
            // Deferred-launch the PTY until the user first opens the session.
            // Launching it eagerly (with no attached client) caused `claude`
            // to exit before writing anything to its session jsonl, leaving
            // a session record whose `--resume` would fail and whose
            // history was empty. Storing pendingInitialPrompt + permission/
            // effort lets the switch_session handler spawn the PTY with the
            // user attached on first open, so claude stays alive and /jira
            // actually runs.
