    ...
    if (!isMate) {
      try {
        ...
        var spawnToolDefs = spawnMcp.getToolDefs(function onSpawn(spawnArgs) {
          var newSess = sm.createSessionRaw({ vendor: spawnArgs.vendor || "claude" });
          newSess.title = String(spawnArgs.title).substring(0, 100);
          ...
          return Promise.resolve({ sessionId: newSess.localId, title: newSess.title });
        });
