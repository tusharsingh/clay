...
function createProjectContext(opts) {
  ...
  // Snapshot a session for the clay-sessions MCP tool responses.
  // Pulls the most recent user/assistant text from history so the caller can
  // see what the child has been doing without paging the full log.
  function summarizeSessionForMcp(session) {
    ...
    for (var i = hist.length - 1; i >= 0; i--) {
      var entry = hist[i];
      if (!entry) continue;
      if (!lastAssistantText && entry.type === "delta" && typeof entry.text === "string" && entry.text.length > 0) {
        lastAssistantText = entry.text;
      }
      if (!lastUserText && entry.type === "user_message" && typeof entry.text === "string") {
    ...
    return {
      ...
      parentSessionId: typeof session.parentSessionId === "number" ? session.parentSessionId : null,
  ...
  var mcpServers = (function () {
    ...
    // Sessions MCP server (available to both mates and main project)
    // Lets the agent fan out work into sibling sessions in the same project,
    // poll their status, and block on their completion.
    try {
      var sessionsMcp = require("./sessions-mcp-server");
      var sessionsToolDefs = sessionsMcp.getToolDefs({
        onSpawn: function (input) {
          var parent = sm.getActiveSession();
          if (parent && parent.loop && parent.loop.active && parent.loop.role !== "crafting") {
            throw new Error("Autonomous loop mode: spawning sibling sessions is disabled.");
          }
          var sessionOpts = {};
          if (input.visibility) sessionOpts.sessionVisibility = input.visibility;
          if (input.vendor) sessionOpts.vendor = input.vendor;
          if (parent && parent.ownerId) sessionOpts.ownerId = parent.ownerId;
          if (parent && typeof parent.localId === "number") sessionOpts.parentSessionId = parent.localId;

          var newSession = sm.createSessionRaw(sessionOpts);
          newSession.title = input.title || input.prompt.slice(0, 60);
          var userMsg = { type: "user_message", text: input.prompt };
          ...
          sdk.startQuery(newSession, input.prompt, undefined, ensureProjectAccessForSession(newSession));

          return { localId: newSession.localId, title: newSession.title };
        },
        onStatus: function (sessionId) {
          var s = sm.sessions.get(sessionId);
          if (!s) return null;
          return summarizeSessionForMcp(s);
        },
        onWait: function (sessionId, timeoutMs, pollMs) {
          var s = sm.sessions.get(sessionId);
          if (!s) return Promise.resolve({ status: null, timedOut: false });
          var deadline = Date.now() + timeoutMs;
          return new Promise(function (resolve) {
            function tick() {
              var current = sm.sessions.get(sessionId);
              if (!current) return resolve({ status: null, timedOut: false });
              if (!current.isProcessing) {
                return resolve({ status: summarizeSessionForMcp(current), timedOut: false });
              }
              if (Date.now() >= deadline) {
                return resolve({ status: summarizeSessionForMcp(current), timedOut: true });
              }
      ...
      var sessionsMcpConfig = adapter.createToolServer({ name: "clay-sessions", version: "1.0.0", tools: sessionsToolDefs });
      if (sessionsMcpConfig) servers[sessionsMcpConfig.name || "clay-sessions"] = sessionsMcpConfig;
    } catch (e) {
      console.error("[project] Failed to create sessions MCP server:", e.message);
    }
