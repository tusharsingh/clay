    ...
    if (!isMate) {
      try {
        ...
        function onMarkDone(args) {
          ...
          var target = null;
          if (sessionId != null) {
            target = sm.sessions.get(sessionId);
          } else if (callingCliSessionId && typeof sm.findSessionByCliSessionId === "function") {
            // The MCP bridge passed through the cliSessionId of the calling
            // session — prefer it over getActiveSession() so a session
            // switch mid-tool-call doesn't move the prefix.
            target = sm.findSessionByCliSessionId(callingCliSessionId);
          }
          if (!target) target = sm.getActiveSession();
          if (!target) {
            return Promise.reject(new Error(
              sessionId != null
                ? ("No session with localId " + sessionId)
                : "No active session to mark — pass session_id explicitly when calling from a dispatcher."
            ));
          }
