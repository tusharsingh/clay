...
function createSessionManager(opts) {
  ...
  function saveSessionFile(session) {
    ...
    try {
      ...
      if (typeof session.parentSessionId === "number") metaObj.parentSessionId = session.parentSessionId;
  ...
  function loadSessions() {
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      session.parentSessionId = typeof m.parentSessionId === "number" ? m.parentSessionId : null;
  ...
  function mapSessionForClient(s, clientActiveId, wsUnread) {
    ...
    return {
      ...
      parentSessionId: typeof s.parentSessionId === "number" ? s.parentSessionId : null,
    };
  }
  ...
  function createSession(sessionOpts, targetWs) {
    ...
    var session = {
      ...
      parentSessionId: (sessionOpts && typeof sessionOpts.parentSessionId === "number")
        ? sessionOpts.parentSessionId : null,
    };
    sessions.set(localId, session);
