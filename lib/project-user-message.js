    ...
    if (msg.type === "term_input") {
      if (msg.id) {
        ...
        if (typeof sm.findSessionByTerminalId === "function") {
          ...
          if (_ts) {
            sm.bumpSessionActivity(_ts.localId);
            // User typed something into a TUI that was flagged as
            // needing attention — that typing IS the response, clear
            // the flag immediately instead of waiting for a session
            // switch. Otherwise the bell pulses forever for users who
            // respond inline without leaving the session.
            if (_ts.needsAttention) {
              _ts.needsAttention = null;
              try { sm.saveSessionFile(_ts); } catch (e) {}
