...
function attachSessions(ctx) {
  ...
  // they render in TUI without converting the on-disk session. Defaults to
  // true in both multi-user and single-user mode.
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "set_always_restart_in_tui") {
      ...
      if (!artUserId) {
        sendTo(ws, { type: "set_always_restart_in_tui_result", ok: false, error: "no_user" });
        return true;
      }
