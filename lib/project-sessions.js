  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "switch_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        if (xmTarget) {
          // Existing sessions always resume in their born mode. The pref only
          // governs the default for NEW sessions (see new_session handler).
          // Close any runtime PTY that the prior cross-mode flow may have left
          // attached so it doesn't leak across the upgrade boundary.
