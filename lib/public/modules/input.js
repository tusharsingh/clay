...
// --- Init ---
// Dispatch the WS message that cancels the in-flight assistant turn.
// Shared by the Send/Stop click handler and the Esc-to-stop keybind so the
// payload lives in one place. Guarded on `readyState === 1` (OPEN) to avoid
// throwing if the socket is CLOSING before `ctx.connected` flips.
function sendStop() {
...
export function initInput(_ctx) {
  ...
  ctx.inputEl.addEventListener("keydown", function (e) {
    ...
    // Esc-to-stop: cancel the in-flight assistant turn. Keep this branch
    // BELOW the slash-menu and mention-menu Esc branches so menu-close
    // wins; both of those return before reaching here. Esc is a no-op
    // when idle, during IME composition, or when the WS is not OPEN.
