...
var collapsedTaskBatchGroups = new Set();
...
function collectItemSessionIds(item) {
  ...
  if ((item.type === "loop" || item.type === "taskBatch") && Array.isArray(item.children)) {
    var ids = [];
...
function renderSessionItem(s) {
  ...
  if (s.taskBatch && s.taskBatch.task && s.taskBatch.task.key) {
    textHtml += '<span class="session-task-key-badge">' + s.taskBatch.task.key + '</span>';
  }
...
function renderTaskBatchGroup(batchId, children) {
  var visibleChildren = children;
  if (searchMatchIds !== null) {
    visibleChildren = [];
    for (var vi = 0; vi < children.length; vi++) {
  ...
  visibleChildren.sort(function (a, b) {
    var ai = (a.taskBatch && typeof a.taskBatch.batchIndex === "number") ? a.taskBatch.batchIndex : 0;
    var bi = (b.taskBatch && typeof b.taskBatch.batchIndex === "number") ? b.taskBatch.batchIndex : 0;
    return ai - bi;
  });

  var first = visibleChildren[0].taskBatch || {};
  var collapsed = collapsedTaskBatchGroups.has(batchId);
  var hasActive = false;
  var anyProcessing = false;
  for (var ci = 0; ci < visibleChildren.length; ci++) {
    if (visibleChildren[ci].active) hasActive = true;
  ...
  var wrapper = document.createElement("div");
  wrapper.className = "session-loop-wrapper session-task-batch-wrapper";

  var headerEl = document.createElement("div");
  headerEl.className = "session-loop-group session-task-batch-group" +
    (hasActive ? " active" : "") + (collapsed ? "" : " expanded");
  headerEl.dataset.batchId = batchId;

  var chevron = document.createElement("button");
  chevron.className = "session-loop-chevron";
  chevron.type = "button";
  chevron.innerHTML = collapsed ? "&rsaquo;" : "&or;";
  chevron.addEventListener("click", function (e) {
    e.stopPropagation();
    if (collapsedTaskBatchGroups.has(batchId)) {
      collapsedTaskBatchGroups.delete(batchId);
    } else {
      collapsedTaskBatchGroups.add(batchId);
    }
    renderSessionList(null);
  });
  ...
  labelEl.className = "session-loop-label";
  var titleText = first.batchTitle || "Tasks";
  if (first.project || first.version) {
    titleText += " · " + (first.project || "") + (first.project && first.version ? " " : "") + (first.version || "");
  }
  titleText += " · " + visibleChildren.length;
  labelEl.textContent = titleText;
  headerEl.appendChild(labelEl);

  if (anyProcessing) {
    var spinner = document.createElement("span");
    spinner.className = "session-processing";
  ...
  if (!collapsed) {
    ...
    childList.className = "session-loop-children";
    for (var i = 0; i < visibleChildren.length; i++) {
      childList.appendChild(renderSessionItem(visibleChildren[i]));
    }
...
export function renderSessionList(sessions) {
  ...
  for (var i = 0; i < cachedSessions.length; i++) {
    ...
    } else if (s.taskBatch && s.taskBatch.batchId) {
      var bId = s.taskBatch.batchId;
      if (!taskBatchGroups[bId]) taskBatchGroups[bId] = [];
      taskBatchGroups[bId].push(s);
    } else {
  ...
  var batchKeys = Object.keys(taskBatchGroups);
  for (var bk = 0; bk < batchKeys.length; bk++) {
    var batchId = batchKeys[bk];
    var batchChildren = taskBatchGroups[batchId];
    var batchMaxActivity = 0;
    for (var bm = 0; bm < batchChildren.length; bm++) {
      var bact = batchChildren[bm].lastActivity || 0;
      if (bact > batchMaxActivity) batchMaxActivity = bact;
    }
    items.push({ type: "taskBatch", batchId: batchId, children: batchChildren, lastActivity: batchMaxActivity });
  }
  ...
  for (var ri = 0; ri < regularItems.length; ri++) {
    ...
    } else if (item.type === "taskBatch") {
      var batchEl = renderTaskBatchGroup(item.batchId, item.children);
      if (batchEl) {
        regularContainer.appendChild(batchEl);
      }
    } else {
