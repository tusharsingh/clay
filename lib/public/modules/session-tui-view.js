...
function syncHostBounds() {
  ...
  // On mobile a fixed-position bottom tab bar overlays the viewport.
  // Subtract its visible height so the xterm input line doesn't sit
  // underneath it. When the keyboard hides the bar (keyboard-hidden
  // class -> display:none), getBoundingClientRect().height is 0 so this
  // becomes a no-op automatically.
  var tabBar = document.getElementById("mobile-tab-bar");
  var tabBarHeight = 0;
  if (tabBar) {
    var tbRect = tabBar.getBoundingClientRect();
    // Only subtract if the bar actually overlaps the host's bottom edge.
    if (tbRect.height > 0 && tbRect.top < visibleBottom) {
      tabBarHeight = Math.min(tbRect.height, visibleBottom - tbRect.top);
    }
  }
  hostEl.style.top = r.top + "px";
  hostEl.style.left = r.left + "px";
  hostEl.style.width = r.width + "px";
  hostEl.style.height = Math.max(0, visibleBottom - r.top - tabBarHeight) + "px";
}
