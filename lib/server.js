  ...
  var appHandler = function (req, res) {
    ...
    if (req.method === "POST" && fullUrl === "/api/tui-notify") {
      ...
      req.on("end", function () {
        ...
        console.log("[tui-notify] matched localId=" + matchedLocalId + " terminalId=" + matchedTerminalId + " project=" + matchedCtx.slug + " ownerId=" + (matchedOwnerId || "(none)") + " -> notification center + sidebar attention");
        // Light up the sidebar's needsAttention indicator on the matched
        // session — same flag the request_user_attention MCP tool sets.
        // Auto-clears when the user switches into the session
        // (switch_session handler clears it). This makes the Claude CLI's
        // Notification hook the primary trigger; explicit
        // request_user_attention calls from skills remain belt-and-suspenders.
        try {
          var sm = matchedCtx.sm;
          if (sm && matchedSession && typeof sm.saveSessionFile === "function" && typeof sm.broadcastSessionList === "function") {
            var reasonText = (message || "").toString().slice(0, 160);
            matchedSession.needsAttention = {
              reason: reasonText,
              requestedAt: Date.now(),
              source: "claude-hook",
            };
            sm.saveSessionFile(matchedSession);
            sm.broadcastSessionList();
          }
        } catch (e) {
          console.log("[tui-notify] failed to set needsAttention: " + e.message);
        }
        try {
